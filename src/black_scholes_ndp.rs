...
use numpy::float64;
pub fn euro_vanilla_call(S: i32, K: i32, T: i32, r: f32, sigma: f32) -> float64 {
...
pub fn euro_vanilla_put(S: i32, K: i32, T: i32, r: f32, sigma: f32) -> float64 {
    let d1 = np.log(S / K) + r + 0.5 * sigma.pow(2) * T / sigma * np.sqrt(T);
    let d2 = np.log(S / K) + r - 0.5 * sigma.pow(2) * T / sigma * np.sqrt(T);
    let put =
...
pub fn euro_vanilla(S: i32, K: i32, T: i32, r: f32, sigma: f32, option: &str) -> float64 {
    let d1 = np.log(S / K) + r + 0.5 * sigma.pow(2) * T / sigma * np.sqrt(T);
    let d2 = np.log(S / K) + r - 0.5 * sigma.pow(2) * T / sigma * np.sqrt(T);
    if option == "call" {
        let mut result =
